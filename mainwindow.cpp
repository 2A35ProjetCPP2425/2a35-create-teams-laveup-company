    ...
    ui(new Ui::MainWindow)
{
    ...
    int ret=A.connect_arduino(); // lancer la connexion à arduino
        ...
        case(-1):qDebug() << "arduino is not available";
         ...
         // Connect the serial port's readyRead signal to the update_label slot
         connect(A.getSerial(), SIGNAL(readyRead()), this, SLOT(update_label()));

         // Initialize the timer for UID timeout handling
         uidTimeoutTimer = new QTimer(this);
         uidTimeoutTimer->setSingleShot(true); // Ensure the timer only runs once
         connect(uidTimeoutTimer, &QTimer::timeout, [this]() {
             uidProcessedRecently = false; // Reset the flag when the timer times out
         });

         // Connect the Arduino serial port readyRead signal to the RFID handling function
         connect(A.getSerial(), &QSerialPort::readyRead, this, &MainWindow::readSerialData);

         ui->statusBar->showMessage("Setup complete."); // Indicate successful initialization
...
void MainWindow::readSerialData()
{
    ...
    if (data.contains("\n")) {
        ...
        if (uid.startsWith("UID:")) {
            uid = uid.mid(4).trimmed();

            // Check if the UID is already processed recently
            if (uid == lastScannedUID && uidProcessedRecently) {
                ui->textEdit->append("Same UID scanned again (ignored): " + uid);
            } else {
                lastScannedUID = uid;
                uidProcessedRecently = true; // Set flag to prevent duplicates
                uidTimeoutTimer->start(3000); // Allow processing of new UID after 3 seconds
                processUID(uid);
            }
        }
    }
